...
public class Spectre extends Enemy {
    ...
    public Spectre(String name, float health, float maxHealth, Map<Move.Status, Integer> statusEffects, List<Move> moves) {
    ...
    public Enemy cloneEnemy() {
        return new Spectre(this.getName(), this.getHealth(), this.getMaxHealth(), new HashMap<>(), this.moves);
    }

    @Override
    public Enemy cloneEnemy(float maxHealth) {
        return new Spectre(this.getName(), maxHealth, maxHealth, new HashMap<>(), this.moves);
    }
}
